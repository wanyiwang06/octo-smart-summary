...
| `DOCUMENT_SUMMARY_SOURCE_API_URL` | Document-source service base URL for persisted document summaries (`POST /api/v1/summaries` with `source_type=4`) and the AI 速览 preview (`POST /api/v1/summaries/document/preview`). Preview requests carrying inline `content` do not use it. When a required by-reference fetch arrives and both this and its fallback are unset, the endpoint answers HTTP 502 / code `50201` | Yes (for document summary and by-reference preview) | — |

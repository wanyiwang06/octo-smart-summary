...
| `DOCUMENT_SUMMARY_SOURCE_API_URL` | Absolute HTTP(S) document-source service base URL for persisted document summaries (`POST /api/v1/summaries` with `source_type=4`) and the AI 速览 preview (`POST /api/v1/summaries/document/preview`). Preview requests carrying inline `content` do not use it. Empty or invalid URLs fail closed: `document_sources` is advertised as `false`, and a required by-reference fetch answers HTTP 502 / code `50201` | Yes (for document summary and by-reference preview) | — |
...
| `MAX_TIME_RANGE_DAYS` | Maximum lookback accepted by the unified summary workspace and enforced by its worker fetch path. Applies to both personal and team workflows created from the workspace. Must be at least `30` and greater than or equal to `DEFAULT_TIME_RANGE_DAYS`. | No | `90` |

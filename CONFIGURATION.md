...
| `REFINE_TIMEOUT` | Total request deadline in seconds for the four summary-refine endpoints (personal/team refine, both streaming and non-streaming). This is the PARENT budget for the whole fallback run, not a per-attempt cap: for a fallback model to get one complete attempt it must satisfy `REFINE_TIMEOUT >= 2 * LLM_TIMEOUT + backoff`, which the 90-second default deliberately does not — the default preserves the latency users see today, and raising it is an explicit latency tradeoff best made against measured refine percentiles (`llm_run_duration_seconds`). Values are clamped to `[1s, 30m]`; unset, unparsable or non-positive values fall back to the default, and any rejection or clamp is logged once per process. | No | `90` |

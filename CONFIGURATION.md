...
| `SUMMARY_MAX_CITATIONS_PER_CLAIM` | Maximum number of `[n]` citation markers a single claim may carry. A "claim" is a maximal consecutive marker run (`[3][7][12]`, horizontal Unicode whitespace tolerated); a newline starts a new claim. Duplicate markers inside one claim are removed first (lossless, keyed on the parsed number so `[1]` and `[01]` are one source) and only then is the cap applied; a capped claim always keeps at least one marker. Enforced on summary-profile agent Map output, summary-profile agent final answers, and the worker's final body (after dedup/orphan-strip, followed by citation-row re-derivation). Prompt input and stream deltas are not capped; the worker reconciles the live view with the final body through a snapshot before `done`. The same resolved number is rendered into the Map, Reduce, and summary-planner prompts. `0` or negative disables capping and removes the cap-specific prompt rule; independent citation-safety fixes remain enabled. | No | `3` |

...
func (p *Processor) executePersonalPipeline(ctx context.Context, task model.SummaryTask, userID string, reportStage func(string), streamDelta func(string) error) (string, []model.Citation, int, int, string, error) {
	...
	if documentMode {
		...
		// combine the real source ordinal with those headings ([3, §14.4],
		// [4.14.1]), creating a precision the citation contract cannot resolve.
		// Collapse only these document-specific shapes before building metadata.

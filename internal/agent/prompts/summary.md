...
- `summarize_chunk` 的 `truncated=true`、`failed_chunk_count>0`、`blank_chunk_count>0` 或 `chunk_calls_capped=true` 表示本次 Map 覆盖不完整（有分块失败、空结果或因分块过多截断了较早的消息）；据此在最终答案中如实说明结论可能不完整，不要当作已覆盖全部内容。

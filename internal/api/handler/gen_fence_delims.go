...
//	   exactly that rune. U+2F42 KANGXI RADICAL SCRIPT ⽂ and U+3246 CIRCLED
//	   IDEOGRAPH SCHOOL ㉆ both fold to 文; U+2F64 KANGXI RADICAL USE ⽤ folds to
//	   用. Round 12 derived the delimiter alphabet but left the tag name as exact
//	   literal runes, so `</⽂档数据>` reached the model byte-identical — the same
//	   failure mode, one alphabet over. Rule C closes it by construction: the
//	   guarded names live here, and re-running the generator finds new members.
//
...
//
// Keep in sync with the newFenceGuard call sites (document_source.go,
// agent_reference_context.go). TestFenceTagNameFoldsCoverGuardedNames pins it.
var guardedTagNames = []string{"文档数据", "引用数据"}

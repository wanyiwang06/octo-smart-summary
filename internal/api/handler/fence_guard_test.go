...
// The <文档数据> guard instance and its two wrappers live with the document preview
// endpoint (document_source.go), which is a separate PR. They are declared here at
// test scope so this PR can exercise the guard on BOTH protected tag names: the
// round-9/10 finding was that the two guards had diverged and fixes landed on one
// while missing the other, so single-name coverage is exactly the gap that let that
// through. When the preview endpoint lands, these move to non-test code unchanged.
...
func sanitizeDocumentFenceText(s string) string { return docFenceGuard.neutralize(s) }

func documentTextWithoutFenceTags(s string) string { return docFenceGuard.strip(s) }
...
func TestFenceGuardStripConverges(t *testing.T) {
	...
	// The property a caller's emptiness check depends on, asserted on strip itself.
	//
	// This was originally observed through the document preview endpoint's emptiness
	// gate (a body of forged tags cleared it and billed a completion: 100 tags gated,
	// 256 billed). That gate belongs to a different PR, so the assertion is stated
	// here in terms of the guard's own contract, which is what the gate consumed:
	// a body of nothing but forged tags strips to empty, at every k that used to trip
	// the pass cap.
	for _, k := range []int{1, 100, 256, 260, 2000} {
		for _, g := range []*fenceGuard{docFenceGuard, refFenceGuard} {
			in := strings.Repeat("<"+g.tagName, k)
			if out := strings.TrimSpace(g.strip(in)); out != "" {
				t.Errorf("%s: a body of %d forged fence tags did not strip to empty: %d runes remain",
					g.tagName, k, utf8.RuneCountInString(out))
			}
		}
	}

	// The other direction: real content next to any number of forged tags must still
	// count as content — strip must not be so aggressive that it empties a real
	// document. This is the constraint that keeps the guard from turning a
	// containment win into silent data loss.
	for _, g := range []*fenceGuard{docFenceGuard, refFenceGuard} {
		in := strings.Repeat("<"+g.tagName, 300) + "真正的正文内容"
		if out := g.strip(in); !strings.Contains(out, "真正的正文内容") {
			t.Errorf("%s: real content was destroyed by strip: %q", g.tagName, out)
		}
	}
}
...
func FuzzFenceGuard(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, in string) {
		...
		// Invariant 1 — the budget invariant. A caller that computes a rune budget
		// BEFORE sanitizing relies on this pass never growing the text; if it breaks,
		// an assembled prompt can exceed the model's context limit.
		...
		// Callers that gate billing on "is this document empty?" depend on this.

...
func TestSummaryWorkspaceRequestedPresetTimeRange(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "natural affirmative after range", message: "最近一个月的进展，总结一下", label: "最近一个月", days: 30, ok: true},
		{name: "polite generation after range", message: "最近一个月的会议记录，帮我生成总结", label: "最近一个月", days: 30, ok: true},
		...
		{name: "explicit question keeps picker", message: "这个总结要包含最近一个月的数据吗", ok: false},
		{name: "draft complaint keeps picker", message: "总结里最近一个月的部分写得不好", ok: false},
		{name: "comparison complaint keeps picker", message: "这份总结和最近一个月的销售数据不一致", ok: false},
		{name: "ignore range keeps picker", message: "总结的时候别管最近一个月的旧消息", ok: false},
		...
		{name: "historical range explicit correction", message: "把之前的时间范围改成最近一周", label: "最近 7 天", days: 7, ok: true},
		{name: "original range explicit correction", message: "原来的范围改成最近一个月", label: "最近一个月", days: 30, ok: true},
		{name: "previous range explicit correction", message: "上次的时间范围改成最近一周", label: "最近 7 天", days: 7, ok: true},
		{name: "former window explicit correction", message: "此前的时间窗口改成最近一个月", label: "最近一个月", days: 30, ok: true},
		{name: "title and range explicit correction", message: "标题和时间范围都改成最近一个月", label: "最近一个月", days: 30, ok: true},
		{name: "keep title but change range", message: "开始总结：标题不改但时间范围改成最近七天", label: "最近 7 天", days: 7, ok: true},
		{name: "incorrect old range explicit correction", message: "之前范围不对现在改成最近七天", label: "最近 7 天", days: 7, ok: true},
		{name: "short shrink command", message: "缩小到一周", label: "最近 7 天", days: 7, ok: true},
